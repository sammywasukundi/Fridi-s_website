<?php include 'models/request.php'; ?>
            ...
            <div class="flex items-center justify-between mx-auto w-[92%]">
                ...
                <ul class="space-x-4 flex justify-end mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400 sm:mt-4">
                    ...
                    <li>
                        ...
                        <button data-modal-target="staticModal" data-modal-toggle="staticModal" data-tooltip-target="tooltip-default" type="button" class="relative inline-flex items-center px-1.5 py-1 mb-4 text-sm font-medium text-center text-blue-400 bg-white rounded-lg hover:bg-blue-400 hover:text-white  border border-blue-300 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-400 dark:hover:bg-blue-400 dark:text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
                            </svg>
                            <?php
                                $read =$pdo->query("SELECT id_message from table_message");
                                $i = 0;
                                while($student=$read->fetch()){
                            ?> 
                            <div class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900"><?= $student['id_message'] ; ?></div>
                            <?php
                                    ++$i;
                                }
                            ?>
                        ...
                        <div id="staticModal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative w-full max-w-2xl max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <!-- Modal body -->
                                    <div class="justify-center p-3 space-y-6">
                                        <div class="w-full max-w-2xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                                            <div class="flex items-center justify-between mb-4">
                                                <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white">Messages d'utilisateurs</h5>
                                        ...
                                        <div class="flow-root">
                                            <?php
                                                $read =$pdo->prepare("SELECT * from table_auteur");
                                                $read->execute();
                                                while($student = $read->fetch()){
                                            ?> 
                                                ...
                                                <li class="py-3 sm:py-4">
                                                    <div class="flex items-center space-x-4">
                                                        ...
                                                        <div class="flex-1 min-w-0">
                                                            ...
                                                            <?php 
                                                                $read =$pdo->prepare("SELECT * from table_message where id_message=?");
                                                                $read->execute(array($_SESSION['id_auteur']));
                                                                while($message = $read->fetch()){
                                                            ?>
                                                            <div class="block text-center space-y-1">
                                                                <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                                                                    <?= $message['message']; ?>
                                                                </p>
                                                                <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                                                                    <a href="fichier_email/<?= $message['monInputFile']; ?>" class="text-blue-400">Ouvrir l'article</a>
                                                                </p>
                                                            </div>
                                                            <?php
                                                                }
                                                        ...
                                                        <div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                                            <img class="w-8 h-8 rounded-full object-cover" src="profils/<?= $student['file']; ?>" alt="Neil image">
                                                        </div>
                                                    </div>
                                                </li>
                                            </ul>
